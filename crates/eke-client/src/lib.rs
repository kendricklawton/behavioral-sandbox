...
//! nothing but the shared wire contract ([`eke_protocol`]) and a JSON value for the opaque trace
//! record.
//!
//! **This is the proof, and the seed.** The proof: it links **no `eke-vmm`**, so it demonstrates
...
//! ([`ClientError`]), a decode fault, a remote [`Error`](eke_protocol::Response::Error), or an
//! unexpected reply, never a panic.
//!
//! ```no_run
//! use eke_client::Client;
