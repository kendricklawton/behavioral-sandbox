...
//! (`eke-vmm` and `eke-probes-loader` tests). Rust compiles each `tests/*.rs` as its own crate,
...
/// engine's own derivation (`jail`'s `MEMORY_OVERHEAD_MIB`, ADR 010), so a test cgroup caps the
...
/// A cgroup carrying the engine's own limit derivation (ADR 010): `cpu.max` = `vcpus` cores,
...
/// (`eke-probes-loader`'s `parse_cap_eff`, which the host path can't share with a dev-only
