...
//! `ekvm serve` daemon (`src/serve.rs`) are both thin hosts of the same `vmm` public API, and
...
/// Firecracker v1.9 caps a microVM at 32 vCPUs, so both the CLI (`--vcpus`) and the
