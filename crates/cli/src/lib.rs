//! Shared library for the `ebpf-kvm-engine` binary: the CLI (`src/main.rs`, `run`/`shell`/…) and the
//! `ebpf-kvm-engine serve` daemon (`src/serve.rs`) are both thin hosts of the same `vmm` public API, and
