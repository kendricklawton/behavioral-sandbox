...
//! `fuzzing` feature exposes exactly those, the same pattern `bsx-channel`
//! uses for its fuzz entry points. The binary compiles the same files as its own modules.
#![forbid(unsafe_code)]
