...
//! not a boundary. The boundary is `ebpf-kvm-engine serve`: its clients arrive over a socket
//! and control neither the daemon's environment nor its `.ebpf-kvm-engine.toml`, so the same policy applied to
...
///
/// Every field is optional/false by default, so an absent `.ebpf-kvm-engine.toml` leaves the engine's existing
...
pub enum PolicyError {
    ...
    Ceiling {
        /// The knob's name as an operator writes it in `.ebpf-kvm-engine.toml`.
        knob: &'static str,
