...
//!
//! The file's shape picks the check. One line is a single envelope; several lines are a **session
...
pub fn run(args: VerifyArgs, sources: &config::Sources) -> Result<ExitCode, CliError> {
    ...
    // The per-envelope size bound stays enforced inside the record crate (`VerifyError::TooLarge`);
    // `read_bounded` bounds the whole file.
...
/// legitimately exceeds the per-envelope bound while every envelope in it stays inside it. A memory
/// bound on the verifying host, since [`verify_chain`] checks the sequence as a unit and reads the
/// whole file.
const MAX_RECORD_FILE_BYTES: usize = 16 * MAX_ENVELOPE_BYTES;

/// A chain is one envelope per line, so the file bound must exceed the envelope bound.
...
/// [`read_bounded`] against an explicit limit, so the boundary is testable without a file the size
/// of the real one. Length is checked on bytes before UTF-8 conversion, so an over-bound file reads
/// as "too large" rather than a misleading encoding error.
...
/// (`BSX_TRUSTED_KEYS` / `.bsx.toml`), and the host's own current signing key, so a record signed
/// *before* a rotation still verifies. Everything reduces to `key_id` hex, so the sources dedup.
