...
//! ([`agent`](../eke-cli/index.html)), the reference client (`eke-client`), and the eventual
//! polyglot SDKs all share, so it lives in its own **`eke-vmm`-free** crate: the wire is the
...
/// while the exec channel still enforces the real `eke_vmm::MAX_PAYLOAD` on the bytes that reach
...
pub enum Request {
    ...
    /// Any omitted field keeps the conservative `eke_vmm::Limits` default.
    Open {
