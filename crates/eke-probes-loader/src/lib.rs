//! `eke-probes-loader`, the userspace side of the eBPF story: load and attach the probes from
...
const OBJECT_ENV: &str = "EKE_PROBES_OBJECT";
...
/// Where the compiled BPF object lives, in precedence order: the `EKE_PROBES_OBJECT` override, the
...
impl ResourceMeter {
    ...
    /// ships alongside the run's [`RunResult`](https://docs.rs/eke-vmm), the CPU figure is meaningful
...
/// and the syscall trace) is the audit record's convergence, kept here, out of `eke-vmm`, so the driver stays
...
mod tests {
    ...
    use eke_probes_common::egress_allowed;
    ...
    fn allow_host6_builds_a_slash128_rule_and_counts_toward_deny_all() {
        ...
        assert!(eke_probes_common::egress_allowed6(
            p.rules6(),
            host.octets(),
            9999,
            Protocol::Udp.as_u8()
    ...
    fn object_path_precedence_lets_a_packaged_install_work_unconfigured() {
        ...
        // EKE_PROBES_OBJECT set. This is what makes an install work unconfigured.
        assert_eq!(
