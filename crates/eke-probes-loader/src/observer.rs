...
//!
//! `eke-vmm` stays independent of this crate (ADRs 021/023/024), so the bundle takes **plain
...
use eke_probes_common::SyscallEvent;
...
impl SandboxProbes {
    ...
    /// (`Sandbox::boot_latency` + `RunResult::metrics.wall`), so the record never depends on `eke-vmm`.
