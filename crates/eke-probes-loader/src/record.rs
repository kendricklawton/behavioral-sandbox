...
//!
//! This module is deliberately dependency-light: no aya, no `eke-vmm`. It defines the shape of
...
//! output will rely on. Kept here, out of `eke-vmm`, so the driver stays independent of the eBPF
//! loader (ADRs 021/023); the two tracks bridge only by plain values.

use std::collections::btree_map::BTreeMap;
use std::time::Duration;

use eke_probes_common::{FlowCounts, FlowKey, FlowKey6, Syscall, SyscallEvent};
...
impl RunRecord {
    /// Assemble a record from already-collected parts. Pure, no eBPF, no `eke-vmm`. This is what
...
/// `Sandbox::boot_latency` and `RunResult::metrics.wall`, so the record never depends on `eke-vmm`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Timing {
    pub boot: Duration,
...
mod tests {
    ...
    fn ev(syscall: u32, cgroup: u64, detail: &[u8], comm: &str) -> SyscallEvent {
        let mut d = [0u8; eke_probes_common::DETAIL_CAP];
        let n = detail.len().min(d.len());
        d[..n].copy_from_slice(&detail[..n]);
        let mut c = [0u8; eke_probes_common::COMM_CAP];
    ...
    fn with_v6_folds_totals_sorts_flows_and_aggregates_denials() {
        use eke_probes_common::IPPROTO_TCP;
        let ula = |n: u8| {
            let mut a = [0u8; 16];
            a[0] = 0xfd;
            a[2] = 0x02;
