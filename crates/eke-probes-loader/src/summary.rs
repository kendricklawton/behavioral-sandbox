...
//! of the existing record, no new observation, no new machinery (ADR 031: the AI-native surface
...
//! itself, out of host view; ADR 029), and the projection does not pretend otherwise.
//!
...
impl RunRecord {
    ...
    /// the record, not new machinery (ADR 031). The leading `schema` field is
...
fn net_summary(out: &mut String, net: &NetSection) {
    ...
    // actually got bytes out, not merely attempted, or a supervising agent (ADR 031) reads a
...
mod tests {
    ...
    use eke_probes_common::{FlowCounts, FlowKey, SyscallEvent, IPPROTO_TCP, IPPROTO_UDP};
    ...
    fn a_denied_endpoint_is_never_listed_as_reached() {
        ...
        // reads a blocked exfil target as reached (ADR 031).
        let record = sample(vec![
