...
//! object). Run via `cargo xtask ci-privileged`. Uses `eke-vmm` as a **dev-dependency only**, so
...
//! cpu/mem/pid caps are proven under real root by the `eke-vmm` confinement suite (a mem-hog /
...
use eke_test_support::{have_real_root, process_threads, LimitCgroup};
use eke_vmm::{BootConfig, Vm, DEFAULT_GUEST_CID, GUEST_READY_MARKER};
