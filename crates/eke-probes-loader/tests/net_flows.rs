...
//! `cargo xtask ci-privileged`. Uses `eke-vmm` as a **dev-dependency only**, so the loader library
...
use eke_probes_loader::{check_support, object_path, TapMonitor};
...
fn guest_traffic_shows_up_in_the_per_vm_counters() {
    ...
    // netns down, which reclaims the tc filter (ADR 020), leaving no dangling host state.
    drop(monitor);
