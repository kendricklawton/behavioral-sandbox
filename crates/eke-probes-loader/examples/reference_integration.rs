...
//! close. This is the launch sequence the driver (`eke-vmm`) and the loader (`eke-probes-loader`)
...
//! `boot_latency`), so the driver never gains a dependency on the eBPF loader (ADRs 021/023);
...
//! cargo build -p eke-probes-loader --example reference_integration
...
use eke_probes_loader::{SandboxProbes, SharedMeter, SharedTracer, Timing};
use eke_vmm::{BootConfig, Limits, Sandbox};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // 2. Configure the run. `from_env` layers flags/env/`.eke.toml`/defaults for the artifact
    ...
    // 3. Boot: hardware isolation (KVM) under the jailer, the confined default (ADR 012).
    ...
    //    at the tap (ADR 022). Each axis that can't attach degrades to a recorded coverage gap.
