//! Integration tests for the guest agent, driving [`guest_agent::serve`] through the **public**
...
use channel::{ClientConnection, Request, Response};
...
fn run(argv: &[&str]) -> (Vec<u8>, Vec<u8>, Result<i32, String>) {
    ...
    let agent = std::thread::spawn(move || guest_agent::serve(guest));
...
fn session_state_persists_across_connections() {
    ...
    let agent = std::thread::spawn(move || guest_agent::serve_session(guest, &session));
    let mut client = ClientConnection::connect(host).expect("client handshake");
    client
        .send_request(&Request::PutFile {
            path: "seed.txt".into(),
