//! The `eke-guest` binary: listen for connections and [`serve`](eke_guest::serve) one command
//! each.
//!
...
//! sentinel ([`GUEST_READY_MARKER`](eke_channel::GUEST_READY_MARKER)) emitted once its vsock
...
use eke_guest_agent::serve_session;
...
fn main() -> ExitCode {
    ...
    let spec = std::env::args()
        .nth(1)
        .or_else(|| std::env::var("EKE_GUEST_LISTEN").ok());
    let Some(spec) = spec else {
        eprintln!("usage: eke-guest <vsock:<port>|unix:<path>>   (or set EKE_GUEST_LISTEN)");
...
fn run_vsock(port: u32) -> Result<(), String> {
    ...
    for conn in listener.incoming() {
        match conn {
            ...
            // (see `eke_guest::serve`). `VsockStream`'s setters return `nix::Error`.
...
/// fires exactly once the vsock listener is accepting. See [`eke_channel::GUEST_READY_MARKER`].
...
/// stderr logging, filter from `EKE_LOG` else `info`. `info` (not the CLI's `warn`) is deliberate:
...
fn init_tracing() {
    let filter = std::env::var("EKE_LOG").unwrap_or_else(|_| "info".to_string());
