//! `eke-guest`, the in-guest agent that runs a command and reports its result over the channel.
//!
...
use eke_channel::{ChannelError, Request, Response, ServerConnection};
...
impl RunDir {
    ...
    fn get(&self, rel: &str) -> Result<Option<Vec<u8>>, AgentError> {
        ...
        match std::fs::metadata(&real) {
            Ok(md) if md.len() > eke_channel::MAX_PAYLOAD as u64 => {
                tracing::warn!(
