...
fn execs_a_command_in_the_microvm() {
    // Closes the provisional "in a microVM" gate: the agent baked into `rootfs-guest.ext4`
...
fn jailed_exec_runs_a_command() {
    ...
    assert_eq!(
        uid.as_deref(),
        Some(vmm::DEFAULT_JAIL_UID.to_string()).as_deref(),
...
#[test]
#[ignore = "needs /dev/kvm + the guest rootfs + the static example (run via `cargo xtask ci-privileged`)"]
