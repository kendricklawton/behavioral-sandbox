...
//! parity so every exposed `bsx_channel::fuzz::decode_*` entry point has a deep target.
//!
...
fuzz_target!(|data: &[u8]| {
    ...
    bsx_channel::fuzz::decode_handshake(data);
    // Correct magic, fuzzed version bytes: the version check and the accept path.
    bsx_channel::fuzz::decode_handshake_after_magic(data);
});
