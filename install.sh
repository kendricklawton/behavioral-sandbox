...
#
# Every statement that touches the filesystem lives in main(), invoked on the last line. `sh` reading
# from a pipe executes as it reads, so a connection that drops mid-transfer would otherwise run a
# prefix of this script; nothing runs until the whole file parses.
# `installer_body_is_deferred_to_a_main_guard` asserts that call stays last and that nothing else
# executes at column zero. Keep new work inside main().
set -eu

# Pinned Firecracker release, single-sourced here for the installer half. `FC_PIN1` (the binary's
# sha256) is checked against `crates/engine/src/doctor.rs` by `install_sh_firecracker_pin_matches_doctor`,
# and `FC_VER` against `crates/engine/src/spawn/fcversion.rs` by
# `install_sh_firecracker_version_is_in_the_pinned_series`. Both stay at column zero: those tests
# match on the line prefix.
