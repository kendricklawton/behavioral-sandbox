...
1. **Check the host.** `cargo xtask setup` (build-time) and `ebpf-kvm-engine doctor` (runtime) report exactly
...
2. **Stand it up.** `cargo xtask self-host` does the whole build: the guest kernel + rootfs + eBPF
   object, installs the `ebpf-kvm-engine` binary, and boots one proof sandbox.
...
3. **Run one sandbox, confined.** With real root you exercise the jailed default (not `--unjailed`):
   `ebpf-kvm-engine run -- python3 -c 'print(2 ** 100)'`. Add `--net` / `--trace` / `--record` / `--watch` to
...
6. **The daemon.** `ebpf-kvm-engine serve --socket ./ebpf-kvm-engine.sock`, then drive it with the reference client or `socat`.
   ([Using the `ebpf-kvm-engine serve` daemon](./daemon.md).)
