...
1. **Check the host.** `cargo xtask setup` (build-time) and `ekvm doctor` (runtime) report exactly
...
2. **Stand it up.** `cargo xtask self-host` does the whole build: the guest kernel + rootfs + eBPF
   object, installs the `ekvm` binary, and boots one proof sandbox.
...
3. **Run one sandbox, confined.** With real root you exercise the jailed default (not `--unjailed`):
   `ekvm run -- python3 -c 'print(2 ** 100)'`. Add `--net` / `--trace` / `--record` / `--watch` to
   see the host-observed record. ([Using the eKVM CLI](./cli.md).)
...
6. **The daemon.** `ekvm serve --socket ./ekvm.sock`, then drive it with the reference client or `socat`.
