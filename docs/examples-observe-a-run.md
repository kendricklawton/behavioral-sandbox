...
enforcement](./probes.md); this page is the workflow. All of these need `/dev/kvm`, the ebpf-kvm-engine
...
```console
ebpf-kvm-engine run --unjailed --net --watch --trace --record run.json -- \
...
- `--record run.json` writes the deterministic JSON record, the machine surface downstream tools
  parse (byte-stable; see [Using the ebpf-kvm-engine CLI](./cli.md)).
...
A typical loop is: run the workload with `ebpf-kvm-engine run --trace` (or `--watch` to see it live, and
