...
enforcement](./probes.md); this page is the workflow. All of these need `/dev/kvm`, the eKVM
...
```console
ekvm run --unjailed --net --watch --trace --record run.json -- \
...
- `--record run.json` writes the deterministic JSON record, the machine surface downstream tools
  parse (byte-stable; see [Using the eKVM CLI](./cli.md)).
...
A typical loop is: run the workload with `ekvm run --trace` (or `--watch` to see it live, and
