...
enforcement](./probes.md); this page is the workflow. All of these need `/dev/kvm`, the eke
...
```console
eke run --unjailed --net --watch --trace --record run.json -- \
...
- `--record run.json` writes the deterministic JSON record, the machine surface downstream tools
  parse (byte-stable; see [Using the eke CLI](./cli.md)).
