...
can't check. `cargo xtask build-rootfs` assembles `rootfs-guest.ext4` from an Alpine minirootfs plus an
...
a byte-identical `rootfs-guest.ext4`. Three non-determinism sources are pinned:
