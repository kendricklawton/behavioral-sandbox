...
- [027. The `.ebpf-kvm-engine.toml` config file layer: nearest-up-from-cwd, env-mirrored keys, typos are errors](./027-the-agent-toml-config-file-layer-nearest-up-from-cwd.md) *(2026-07-17)*
- [028. `ebpf-kvm-engine doctor` shares one host-check implementation; the JSON surfaces are versioned before anyone parses them](./028-agent-doctor-shares-one-host-check-implementation-the.md) *(2026-07-17)*
...
- [030. The wire API is versioned newline-JSON in a shared `protocol` crate, not gRPC](./030-the-wire-api-is-versioned-newline-json-in-a-shared.md) *(2026-07-17)*
...
- [042. Hoster admission: a typed capacity refusal, resource-aware admission, committed-resource telemetry](./042-hoster-admission-typed-capacity-refusal-resource-aware.md) *(2026-07-24)*
