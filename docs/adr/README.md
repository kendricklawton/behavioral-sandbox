...
- [027. The `.ekvm.toml` config file layer: nearest-up-from-cwd, env-mirrored keys, typos are errors](./027-the-agent-toml-config-file-layer-nearest-up-from-cwd.md) *(2026-07-17)*
- [028. `ekvm doctor` shares one host-check implementation; the JSON surfaces are versioned before anyone parses them](./028-agent-doctor-shares-one-host-check-implementation-the.md) *(2026-07-17)*
