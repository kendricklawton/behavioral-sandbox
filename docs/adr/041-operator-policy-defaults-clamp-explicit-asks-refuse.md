...
code. `.ekvm.toml` covered **where things live** (`kernel`, `rootfs`, `firecracker`, `scratch_dir`),
...
- **The daemon is the boundary.** `ekvm serve`'s clients arrive over a socket and control neither its
  environment nor its config, so bounding a client's `open` is real enforcement. Its policy comes from
  **explicit flags** (`--max-vcpus` and friends), never a discovered `.ekvm.toml`: a daemon must not
...
- **Just add more `.ekvm.toml` keys.** Rejected: under flags > env > file, a caller overrides every
...
- **Two vocabularies, deliberately.** The CLI reads `.ekvm.toml`; the daemon takes flags. Documented,
...
`Limits`), with `--max-vcpus`/`--max-mem-mib`/`--max-wall-secs`/`--max-output-cap` on `ekvm serve`;
the CLI resolves the same way from `.ekvm.toml` in `crates/cli/src/main.rs`, plus `require_jail` and
`allow_net`.
