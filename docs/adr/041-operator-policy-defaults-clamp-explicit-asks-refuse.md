...
code. `.ebpf-kvm-engine.toml` covered **where things live** (`kernel`, `rootfs`, `firecracker`, `scratch_dir`),
...
- **The daemon is the boundary.** `ebpf-kvm-engine serve`'s clients arrive over a socket and control neither its
  environment nor its config, so bounding a client's `open` is real enforcement. Its policy comes from
  **explicit flags** (`--max-vcpus` and friends), never a discovered `.ebpf-kvm-engine.toml`: a daemon must not
...
- **Just add more `.ebpf-kvm-engine.toml` keys.** Rejected: under flags > env > file, a caller overrides every
...
- **Put policy in `vmm`.** Rejected: an embedder constructs `Limits` directly and *is* the
...
- **Two vocabularies, deliberately.** The CLI reads `.ebpf-kvm-engine.toml`; the daemon takes flags. Documented,
...
`Limits`), with `--max-vcpus`/`--max-mem-mib`/`--max-wall-secs`/`--max-output-cap` on `ebpf-kvm-engine serve`;
the CLI resolves the same way from `.ebpf-kvm-engine.toml` in `crates/cli/src/main.rs`, plus `require_jail` and
`allow_net`.
