...
code. `.eke.toml` covered **where things live** (`kernel`, `rootfs`, `firecracker`, `scratch_dir`),
...
- **The daemon is the boundary.** `kee serve`'s clients arrive over a socket and control neither its
  ...
  **explicit flags** (`--max-vcpus` and friends), never a discovered `.eke.toml`: a daemon must not
...
- **Just add more `.eke.toml` keys.** Rejected: under flags > env > file, a caller overrides every
...
- **Put policy in `eke-vmm`.** Rejected: an embedder constructs `Limits` directly and *is* the
...
- **Two vocabularies, deliberately.** The CLI reads `.eke.toml`; the daemon takes flags. Documented,
...
the CLI resolves the same way from `.eke.toml` in `crates/cli/src/main.rs`, plus `require_jail` and
`allow_net`.
