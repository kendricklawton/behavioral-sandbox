...
one program per sandbox, but the scheduler tracepoint is global, so that path pays O(sandboxes) on every
context switch. This decision fixes both *how* the cost is measured and *how it scales*.
...
- **A target *set* (`METER_TARGETS`), not a program-per-sandbox.** `sched_switch` is a *global*
  ...
  bounded and sane under many concurrent sandboxes (measured by `bench-meter`). A `METER_ALL`
...
- **Correlated by the Firecracker per-VM cgroup.** `cgroup_id_of_pid(vmm_pid)` resolves the id for the
  ...
  `ResourceMeter::summary_for_pid` rolls them into a `ResourceSummary`.

**Alternatives considered.**
- **Read only cgroup v2 files, no eBPF.** Rejected as the CPU story: `cpu.stat` gives a coarse total,
  but the goal is resource accounting **via cgroup-bpf**, and the scheduler tracepoint gives precise,
...
- **Not the pinned public API.** The surface is on `probes-loader` (`ResourceMeter`, `CgroupStats`,
  ...
  and the syscall trace) is a later convergence, kept out of `agent-vmm` so the driver stays
