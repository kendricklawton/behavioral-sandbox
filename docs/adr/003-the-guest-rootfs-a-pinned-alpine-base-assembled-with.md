...
**Context.** The engine runs untrusted code inside a Firecracker microVM, and that guest needs a
userland: a rootfs that boots, carries the exec surface, and grows to real language runtimes without
becoming a second trust boundary. Several standing forces shape how it is built. It must be
*runtime-agnostic*, a base that boots minimally yet scales to Python and Node. It must be built
reproducibly and rootlessly (no `sudo cargo` roulette), so the image is a pinned artifact rather than
whatever a daemon happened to export. It must not disturb the pinned Ubuntu boot rootfs used to bring
the microVM up, whose `ci-privileged` sha256 hash-guard and `login:` boot test are load-bearing. And
the in-guest agent that carries exec/IO must stay convenience, never the thing that contains the guest
(core property 2). Those forces, not a discovered defect, drive the choices below.
...
in at `/usr/local/bin/guest-agent`, installs a minimal init, and assembles an ext4 image
(`artifacts/rootfs-guest.ext4`) with **`mke2fs -d`**, populating the filesystem from a staging dir
...
- **Init model: busybox `init` is PID 1**, with a custom `/etc/inittab` (replacing Alpine's OpenRC)
  ...
  (`VSOCK_PORT`) attached to `ttyS0`. The agent is deliberately **not** PID 1: it has no
...
- **Readiness contract: the agent emits the sentinel, post-`bind`.** The agent prints
  `GUEST_READY_MARKER` (`channel`) to stdout, the serial console, *after* its vsock listener
...
- **Scratch + a static busybox.** Most minimal and educational, but no `/etc` skeleton, no musl
  loader, no package manager, and the next capabilities (Python, Node) want a real libc
  userland; static CPython on scratch is genuinely painful. Rejected as the base; the scratch approach
  survives in the static Go/Rust ELF path, which runs on this same image.
...
- **Overwrite `rootfs.ext4` / flip `BootConfig::default().rootfs` to Alpine.** Tempting ("`exec`
  ...
  `login:` boot test in the same change. Kept **additive**: distinct filename, the test points at
...
via `apk`, the pragmatic base for the *runtime-agnostic* rootfs the guest calls for.
...
boundary) preserves core property 2. This closes decision 002's coupling between the channel and this
rootfs and its `vhost-vsock` prerequisite: the pinned Firecracker CI kernel (`vmlinux-6.1.102`)
carries the guest vsock transport + `CONFIG_DEVTMPFS_MOUNT`, proven by the in-VM
`exec("echo hi") → hi, exit 0` round trip.
...
- **A default-rootfs flip (Alpine replaces Ubuntu as the boot default) is a separate future change**,
  touching the default marker, the `ci-privileged` guard, and the boot test together.
