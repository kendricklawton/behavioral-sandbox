...
earlier decision pulled the network half forward observe-only.: a `--trace` run watches
...
., the policy is live before the guest's traffic path is.

**Decision.** `ekvm run --allow IP[/CIDR][:PORT][/PROTO]` (repeatable, `requires` `--net`) projects
...
tc programs go live (the no-unpoliced-window property). Every allowance is explicit on
...
(a `--trace` run still works). A *policy* can't: a run that asked to enforce one and
...
`ekvm doctor`, and the JSON schema version remain. `--allow` is `run`-only, where `--net` lives (the
