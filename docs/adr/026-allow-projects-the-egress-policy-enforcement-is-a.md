...
**Decision.** `ebpf-kvm-engine run --allow IP[/CIDR][:PORT][/PROTO]` (repeatable, `requires` `--net`) projects
...
`ebpf-kvm-engine doctor`, and the JSON schema version remain. `--allow` is `run`-only, where `--net` lives (the
