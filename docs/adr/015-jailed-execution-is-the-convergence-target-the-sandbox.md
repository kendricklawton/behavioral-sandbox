...
put the walls in tension. The jailer runs on a plain read-write cold boot, and decisions 012/013 make a
...
- The `Limits`/`Sandbox` work assumes the jailed exec path exists; `require_limits` (decision 013's
...
- Jailed snapshot/restore and the pre-warmed pool under the jailer remain downstream of exec under the
  jailer (a jailed VM's disk lives in the chroot, decision 010), tracked with the same boxes.
...
- In the current tree the convergence is complete: `jail` composes with every boot feature and with
  ...
  typed refusal, snapshot an unjailed pre-warmed source, restore jailed clones (decision 010
