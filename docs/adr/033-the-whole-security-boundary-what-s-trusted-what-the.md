...
pieces: "isolation is hardware" as a core property, decision 016's engine/hoster line, decision 022's
...
plus the derived per-drive **IO-bandwidth bound** (decision 013's "derived defaults, not `Limits`
knobs", a virtio-blk rate limiter so a disk-thrashing guest can't starve a co-resident run); network
exfiltration/flood → deny-by-default egress enforced at the tap (decision 025); observation evasion →
the guest can't reach host-kernel eBPF; leak-on-death → the cgroup-owned lifetime + sweep (decisions
014/016); clone state-bleed → per-clone overlay + RAM. The consolidated proof is that these hold
