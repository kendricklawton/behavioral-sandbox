...
each file's hash. Setting `EBPF_KVM_ENGINE_VENDOR_DIR` to that dir takes every build path offline in one move:
...
- **The vendor-aware seam is `fetch_one`, not the call sites.** `fetch_one` branches on
  `EBPF_KVM_ENGINE_VENDOR_DIR` (restore-from-mirror vs `download_one`); `build-rootfs`, `fetch-artifacts`, and
