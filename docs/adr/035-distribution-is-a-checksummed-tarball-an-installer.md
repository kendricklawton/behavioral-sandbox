...
- **A release tarball + `SHA256SUMS`.** `dist` builds the release binary and the three runtime
  ...
  source tree), stages them as `bin/` + `share/ebpf-kvm-engine/`, writes a per-file `MANIFEST.sha256` inside
...
`EBPF_KVM_ENGINE_*` env (repo, version, tarball, prefix, data dir), so the rename sweep is a defaults change.
Verification is testable offline end to end (`EBPF_KVM_ENGINE_DIST_TARBALL` mode), which is also how the
