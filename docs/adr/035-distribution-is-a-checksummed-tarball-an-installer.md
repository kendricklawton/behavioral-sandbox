...
**Context.** The engine is self-hostable from source., but "others can run it" also
...
the contract for every pinned upstream., and a shipped package that is *less*
...
- **A release tarball + `SHA256SUMS`.** `dist` builds the release binary and the three runtime
  ...
  source tree), stages them as `bin/` + `share/ekvm/`, writes a per-file `MANIFEST.sha256` inside
...
- **`install.sh`**, the `curl | sh` face, also packed into the tarball so a hand-downloaded package
  ...
  `$XDG_DATA_HOME/ekvm` (default `~/.local/share/ekvm`), and a starter `~/.ekvm.toml`
  ...
  discovery. makes it apply anywhere under `$HOME`. Firecracker stays the host's to install
., except:
...
- **Bundling Firecracker in the tarball.** Rejected: on a host, the VMM is a host prerequisite the
  hoster patches on their own cadence.; the container is the exception because its
...
`EKVM_*` env (repo, version, tarball, prefix, data dir), so the rename sweep is a defaults change.
Verification is testable offline end to end (`EKVM_DIST_TARBALL` mode), which is also how the
