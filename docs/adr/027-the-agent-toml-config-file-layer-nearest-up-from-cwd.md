# 027. The `.ekvm.toml` config file layer: nearest-up-from-cwd, env-mirrored keys, typos are errors *(2026-07-17)*

**Context.** The engine's configuration resolves through the precedence
`flags > env (EKVM_*) > file > defaults`, and the file layer is the piece that lets a project pin its
...
**Decision.** The file layer sits between the environment and the defaults as a `.ekvm.toml` **file**.
Discovery is the **nearest `.ekvm.toml` walking up from the cwd** (the `.gitignore`/`.editorconfig`
convention), so a project pins its engine config beside its code and a nearer file shadows a farther one.
The file's keys **mirror the `EKVM_*` env names 1:1** (minus the prefix, lowercased: `kernel`, `rootfs`,
