# 027. The `.kee.toml` config file layer: nearest-up-from-cwd, env-mirrored keys, typos are errors *(2026-07-17)*

**Context.** The engine's configuration resolves through the precedence
`flags > env (KEE_*) > file > defaults`, and the file layer is the piece that lets a project pin its
...
**Decision.** The file layer sits between the environment and the defaults as a `.kee.toml` **file**.
Discovery is the **nearest `.kee.toml` walking up from the cwd** (the `.gitignore`/`.editorconfig`
convention), so a project pins its engine config beside its code and a nearer file shadows a farther one.
The file's keys **mirror the `KEE_*` env names 1:1** (minus the prefix, lowercased: `kernel`, `rootfs`,
...
`kee-vmm::BootConfig::from_env_with` (made public for this) takes a lookup closure; the CLI composes
...
Making `from_env_with` public is an additive change to `kee-vmm`, not to the enumerated pinned items
