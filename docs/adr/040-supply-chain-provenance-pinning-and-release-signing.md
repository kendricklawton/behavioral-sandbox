...
- **Pin and verify the Firecracker/jailer binary by sha256.** A pinned hash lives alongside the
  ...
  `ebpf-kvm-engine doctor` gains a check on the installed binary's hash. That check is **advisory**: an operator
...
- **Two boxes, tracked.** The Firecracker-binary pin (`xtask/src/artifacts.rs` + `install.sh` +
  `ebpf-kvm-engine doctor`) and the `SHA256SUMS` signing (`xtask/src/dist.rs` + `install.sh`) are their own
