...
boundary. That makes the supported platform a *security* property, not a convenience
...
- **Host kernel: a security-maintained LTS floor, `MIN_KERNEL` (currently 5.15)**, a **hard** floor, not
  ...
  `cgroup.kill` requirement.; it does not exclude common fleets (Ubuntu 22.04 ships 5.15).
...
- **Firecracker: pinned v1.9., a degradation off-pin**, a different version boots with a
...
- **cgroup v2 controller delegation stays a *degradation***.: resource caps are fairness
...
.: no BTF/caps degrades `--trace`/`--watch` to a coverage gap, but `--allow`
