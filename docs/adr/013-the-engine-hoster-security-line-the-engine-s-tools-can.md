...
- **The hoster owns deployment, as whom, when, over what, and how a shared resource is divided.** Four
  ...
  (3) *harden the scratch base* (point `EKVM_SCRATCH_DIR` at an engine-user-owned dir so no decoy can be
...
- **Have the engine harden the base itself** (refuse a world-writable scratch dir, or `chmod` it).
  ...
  risk the engine already neutralizes. Surfaced as a hardening *recommendation* in `ekvm setup` instead.
...
**Consequences.**
- Surfaced where a self-hoster looks: `ekvm setup` prints a "Hardening, the hoster's responsibility"
...
- The engine/hoster split now has a concrete precedent to reuse: any future privileged tool
  (a future `ekvm gc`, daemon-side reconcilers) inherits the same "authorship not policy, euid-scoped,
