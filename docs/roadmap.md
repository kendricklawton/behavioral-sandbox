...
Order here is rough priority, not a promise. An entry ships only with a working demo (and
measured percentiles, where it claims performance); until then it is intent, not a feature.
...
snapshot's pages copy-on-write instead of each holding a private copy. A claim like "dozens of
exec-ready sandboxes in under a second, most memory shared" ships only with measured
percentiles.
