...
```console
$ echo 'hello' | cargo run -q -p kee-cli -- run -- \
...
not an error (death by signal comes back as `128 + signal`); exit code `2` from `kee run` itself
means the engine failed to stand the run up.

```console
$ cargo run -q -p kee-cli -- run --json -- python3 -c 'print(2 + 2)' 2>/dev/null
...
```console
$ cargo run -q -p kee-cli -- run --json -- python3 -c 'print(2 + 2)' 2>/dev/null | jq .exit_code
0
```
...
```console
$ cargo run -q -p kee-cli -- run --put ./mytool -- /bin/sh -c 'chmod +x mytool && ./mytool'
```
