...
```console
$ echo 'hello' | cargo run -q -p cli -- run -- \
...
```console
$ cargo run -q -p cli -- run --json -- python3 -c 'print(2 + 2)' 2>/dev/null
...
```console
$ cargo run -q -p cli -- run --json -- python3 -c 'print(2 + 2)' 2>/dev/null | jq .exit_code
0
```
...
```console
$ cargo run -q -p cli -- run --put ./mytool -- /bin/sh -c 'chmod +x mytool && ./mytool'
```
