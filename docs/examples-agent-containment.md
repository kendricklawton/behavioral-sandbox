...
Needs `/dev/kvm`, the eke rootfs (`cargo xtask build-rootfs`), the built probe object
...
signed with the host key, so `kee verify record.json` catches any tampering *after* the run, not
...
[Using the eke CLI](./cli.md#watching-a-run-from-the-host) for all four faces of the one record.

## Over the wire, too

An agent driving the daemon reads the same projection, not a CLI-only convenience: `kee` serves it
...
[Using the eke daemon](./daemon.md). (Daemon sessions are observe-only, the `--allow`
