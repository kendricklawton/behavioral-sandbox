...
envelope), the daemon wire in `crates/protocol` (random bytes with injected newlines,
...
string accessors). `protocol` uses `serde_json` (already a dependency) to build valid seeds;
...
`fuzzing` feature (`channel::fuzz`, `protocol::fuzz`) or the already-public parse surface
(`probes-loader`, `probes-common`), without changing any default build or the wire
...
```console
cargo test -p channel                # includes the fuzz_tests property suite
cargo test -p probes-loader --lib    # includes the envelope mutation test
cargo test -p protocol               # includes the daemon-wire property suite
cargo test -p probes-common          # includes the eBPF-boundary parser property test
```
