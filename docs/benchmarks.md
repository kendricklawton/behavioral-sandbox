...
numbers below come from the benchmark suite in [`xtask`](https://github.com/packsixfour/ebpf-kvm-engine/tree/main/xtask), re-run with `cargo xtask bench-all`.
...
- **Reproduce.** One command runs the whole suite as a single report:
  ...
  The KVM benches need `/dev/kvm` + the built ebpf-kvm-engine rootfs; the eBPF benches need
...
15 GiB RAM**, ebpf-kvm-engine rootfs 132 MiB, guest 256 MiB / 1 vCPU. Your hardware will differ; re-run the
