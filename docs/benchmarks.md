...
The commit is `575d1dd`. `xtask/rootfs-packages.lock` was re-pinned on the measurement host to the
...
`575d1dd` are the ones measured. The second arm is `575d1dd` with the token `quiet` deleted from
