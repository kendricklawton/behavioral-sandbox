...
in [`SECURITY.md`](https://github.com/packsixfour/ebpf-kvm-engine/blob/main/SECURITY.md) at the repo root
...
*after* the producing host. Verify a record with `ebpf-kvm-engine verify <record>` (or against the trusted
...
- A signed record that verifies **after** being altered, or a forged signature accepted by
  `ebpf-kvm-engine verify` without the host key (the record-integrity guarantee, decision 034).
...
- **Hosts below the supported floor.** An unsupported architecture or a host kernel older than the
  LTS floor is refused by `ebpf-kvm-engine doctor` (decision 032); weaknesses that require running there
...
[`SECURITY.md`](https://github.com/packsixfour/ebpf-kvm-engine/blob/main/SECURITY.md) (private GitHub
...
3. **Disclose together.** The timeline is agreed with the reporter in the advisory; the default
   ...
   [`RELEASES.md`](https://github.com/packsixfour/ebpf-kvm-engine/blob/main/RELEASES.md) notes it, and
