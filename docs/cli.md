...
**stdout**, so `ekvm run … 2>/dev/null` stays pipe-clean and `--json | jq` just works. The
guest command's exit code becomes `ekvm run`'s own (a crash *inside* the sandbox is a result,
...
records already signed. Keep the retired public keys (their `key_id`s) listed in `EKVM_TRUSTED_KEYS`
(comma-separated) or `trusted_keys` in `.ekvm.toml`, and `ekvm verify` trusts that set together with
...
anchor (the append-only limitation).
...
| Variable | `.ekvm.toml` key | What it points at | Default |
|----------|-------------------|-------------------|---------|
...
trusted. The real boundary is [`ekvm serve`](./daemon.md), whose clients control neither the daemon's
...
|-------------------|-------------|
| Boot + one exec | `ekvm run -- <cmd>` |
| Stateful session | `ekvm shell` |
...
| Verify a signed record | `ekvm verify <record>` |
| Structured run result | `--json` |
| Host readiness | `ekvm doctor` |
| Config layering | flags > env (`EKVM_*`) > `.ekvm.toml` > defaults |
...
- **Snapshots + the pre-warmed pool**, a pre-warmed pool is a long-lived-process concern; it lives
  in the [`ekvm` daemon](./daemon.md) (`--prewarm`), not a one-shot CLI.
