...
Both live in `crates/engine/src/spawn/fcversion.rs`, which holds the version probe and nothing else:
...
`crates/engine/src/spawn/fcversion.rs`). Sending it unconditionally silently drags the real floor up to that field's
