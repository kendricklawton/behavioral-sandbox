...
- **`crates/probes-loader`** is the userspace side, built with **aya** (pure-Rust, no libbpf/C
  ...
  output, or `EKE_PROBES_OBJECT`), never `include_bytes!`/`build.rs`, so the host workspace stays on
...
guest. It lives in
`probes-loader` (not `eke-vmm`, decisions 021/023/024), bridged to the driver only by plain values:
...
(`kee run --net --trace --record --watch`) is documented in [Using the eke CLI](./cli.md); decision
...
cargo build -p eke-probes-loader --example count_execve
...
```console
cargo test -p eke-probes-loader --test counter --no-run
...
`/dev/kvm` + the eke rootfs + the built object, run as root or with the named caps):
...
```console
cargo build -p eke-probes-loader --example trace_syscalls
