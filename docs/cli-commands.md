...
| `--gateway IP` | Give the guest a default route via this address, normally the host end of its /30. Names a path rather than creating one: the engine builds no uplink, so where nothing has furnished the netns the reachable set is unchanged. What it changes is that the attempt now crosses the tap, so `--allow` can bound it and the record can show it. Requires `--net`; see [decision 9](./architecture-decisions.md#9-egress-is-enabled-by-the-engine-constructed-by-the-hoster). |
