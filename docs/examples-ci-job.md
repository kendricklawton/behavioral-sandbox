...
The job script is [`ci-job.sh`](https://github.com/k-henry-org/ebpf-kvm-engine/blob/main/docs/examples/ci-job.sh):
...
```console
$ cargo run -q -p cli -- run --unjailed \
...
```console
$ cargo run -q -p cli -- run --unjailed --net \
