...
- **No platform API.** The programmatic surface is the Rust library and the CLI. Anything that
...
`~/.bsx.toml`'s `jail_uid` select for operators who separate tenants by running a process each
rather than by spanning one. The id is read from the user's own file rather than one found above the working
...
The `bsx-engine` public library API and the host↔guest wire framing are the surface the project
intends to pin as its stability boundary. This list and `AGENTS.md`'s `api`-scope rule name the same
surface, since a commit scope that does not match the policy it audits is worse than no scope at
all:
