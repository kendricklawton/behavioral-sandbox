...
**host-signed** (so alteration after the run is detectable off-host, verify it with `ebpf-kvm-engine verify`),
...
that; `cargo xtask setup` (or `ebpf-kvm-engine doctor` once built) then reports exactly what your host is still
...
- **[Using the ebpf-kvm-engine CLI](docs/cli.md)**, how to run the engine:
  [installation](docs/cli-install.md), building the guest artifacts, `ebpf-kvm-engine run`, `ebpf-kvm-engine shell`.
...
record is surfaced through the CLI (`--trace`/`--record`/`--watch`) and the `ebpf-kvm-engine serve` daemon.

**Pre-1.0 and unstable, expect breaking changes.** Until the first stable release, nothing here
carries a compatibility guarantee: the `Sandbox`/`vmm` API, the `ebpf-kvm-engine serve` wire protocol (and its
...
and a privileged CI lane behind it (decision 032). `ebpf-kvm-engine doctor` reports your own host's
...
| `crates/guest-agent` | The in-guest agent (`guest-agent`): runs one command per connection, streams stdout/stderr/exit. Exec/IO only, never the trust boundary. |
...
| `crates/cli` | The `ebpf-kvm-engine` CLI (also installed as `ebpf-kvm-engine`): `run`, `shell`, `doctor` plus the `ebpf-kvm-engine serve` driver daemon. |
