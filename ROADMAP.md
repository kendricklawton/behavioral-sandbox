...
- [x] **P3.1** Reproducible **rootfs build**: a minimal ext4 image (busybox/alpine or a scratch
      ...
      *(`cargo xtask build-rootfs` → `artifacts/rootfs-guest.ext4`: a sha256-pinned Alpine
...
- [x] **P3.6** Pin the rootfs build in `xtask` so it's one command + reproducible.
      *(`cargo xtask build-rootfs` is now **byte-for-byte deterministic** (decision 007): two builds
      produce an identical `rootfs-guest.ext4`. `SOURCE_DATE_EPOCH` (fixed, scoped to `mke2fs`) stamps
...
- [x] **P4.9a** Guest v6 stack + host↔guest routing. *(Done: the guest kernel drops `ipv6.disable=1`
      ...
      `guest_ip6=<addr>/<plen>` kernel-cmdline token (`ip=`/`CONFIG_IP_PNP` is v4-only): a
      `/sbin/net-up` sysinit step reads it from `/proc/cmdline` and assigns it, best-effort (`ip` then
      `ifconfig`). The token key is single-sourced in `channel` so the driver's writer and the
...
- [x] **P4.9b** Observers parse IPv6 (ADR 018/020). *(Done: `probes-common` gained the v6 twins
...
- [x] **P5.6** `Pool` that keeps pre-warmed restores ready so `exec` starts in ms. *(First pre-warmed-pool/retry
      ...
      *(`vmm::Pool` (`pool.rs`): prefill `target` clones from one pre-warmed snapshot; `take()` pops
...
- [x] **P6.9a** Orphan sweep (the runtime's GC). Decision 011 leaves a crashed driver's scratch dirs
      ...
      *(`vmm::sweep_orphans(scratch_base) -> SweepReport` (`sweep.rs`). **Ownership is the
...
- [x] **P6.9c** The per-sandbox fd budget, measured and stated. Each live VM holds several host fds
      ...
      *(**Measured: 2 fds per live VM, on all three start paths** (cold, networked, pre-warmed restore;
      ...
      held by the driver. Published as `vmm::FDS_PER_VM = 8` (budget deliberately above the
...
- **Exit gate:** a Rust eBPF program loads and reports.
  *(Passed: the demo is `probes-loader`'s `count_execve` example (loads, attaches, reports the
...
- [x] **P10.6** Test: traffic from a guest shows up in the per-VM counters.
      *(Landed: the `#[ignore]`d `guest_traffic_shows_up_in_the_per_vm_counters` boots a networked agent
      ...
      nonzero. Uses `vmm` as a dev-dependency only, so the loader library stays decoupled.)*
...
- [x] **P11.7** Test: a guest can reach an allow-listed endpoint and is blocked from everything else.
      *(Landed: the `#[ignore]`d `a_guest_reaches_the_allow_listed_endpoint_and_is_blocked_from_the_rest`
      ...
      the allowed flow shows in the counters (sent and let through). `vmm` a dev-dependency only.)*
...
- [x] **P12.3** Expose a per-run resource summary in the run result.
      *(Landed: `ResourceSummary { cpu_time, cgroup: CgroupStats }`, the eBPF-measured CPU plus the
      ...
      `vmm` so the driver stays independent of the eBPF loader. Decision 023.)*
...
- [x] **P12.5** Test: a CPU-heavy run reports higher CPU than an idle one, attributed correctly.
      *(Landed: the `#[ignore]`d `a_cpu_heavy_run_reports_more_cpu_than_an_idle_one_attributed_to_the_sandbox`
      ...
      exclusivity is the attribution proof. `vmm` a dev-dependency only.)*
...
- [x] **P13.1** On `Sandbox::open`, attach syscall + network + accounting probes bound to that VM.
      *(Landed: `probes-loader`'s new `observer` module, `ArmedProbes::arm()` (pre-boot: load the
      ...
      sequence, kept **out of `vmm`** (bridged only by the plain values `Sandbox` exposes:
...
- [x] **P13.2** Aggregate into one per-run record: network flows, resources, egress denials, timing,
      ...
      *(Landed: `probes-loader`'s new `record` module, `RunRecord { network: Option<NetSection>,
...
- [x] **P13.6** Test: run a workload that touches network + files → the record shows exactly that.
      *(Landed: the `#[ignore]`d `a_networked_file_touching_run_yields_a_faithful_audit_record`
      ...
      guest's syscalls. `vmm` a dev-dependency only.)*
...
- [x] **P15.1** Adversarial suite: guest tries to escape/DoS/exfiltrate → contained + recorded.
      *(Landed as `probes-loader/tests/hardening.rs::a_hostile_guest_is_contained_and_the_record_shows_it`:
      ...
      cpu/mem/pid caps under real hostile load are the `vmm` confinement suite's real-root
...
- [x] **P16.2** A **versioned** wire API (JSON/gRPC, `(decision)`): open/exec/put/get/snapshot/
      ...
      *(Decision 030: newline-JSON, **not gRPC**, the daemon is synchronous with no async runtime,
      ...
      from the audit-record and `--json` schemas). Lives in a new **`protocol`** crate
      (serde-only, **no `vmm`**): the shared `Request`/`Response`/`Envelope` shapes + a bounded,
...
- [x] **P16.4** A **reference (Rust) client** proving a non-Rust caller can drive `ekvm` over the
      ...
      *(New **`client`** crate: a `Client` driving the whole session (`open`/`exec`/`put`/`get`/
      `snapshot`/`trace`/`close`) with typed errors, no panics. Depends on `protocol` + a JSON
      value **only, never `vmm`**, that dependency set *is* the proof a caller needs nothing of
...
- [x] **P16.6** Explicitly document the non-goals again at the API layer (no tenancy/auth/billing).
      *(Restated where the programmatic interface actually lives: a **crate-level non-goals paragraph**
      on `protocol` (the wire is the SDK contract, so the API surface is the code itself),
...
- [x] **P17.6** Find + fix the top bottleneck the numbers reveal.
      *(The P17.1 latency decomposition made the bottleneck legible: the driver's three readiness waits
      (`await_api_socket`, `await_userspace`, and restore's `await_guest_ready`) polled on a **fixed
...
- [x] **P18.3** The projection joins the **wire API** (P16.2), so the daemon serves it and the
      ...
      *(New `trace_summary` verb in `protocol`, parallel to `trace`: `Request::TraceSummary` →
      ...
      `client` gains `trace_summary()`; the `Verb` metrics enum + `agent_e2e` (raw-JSON *and*
