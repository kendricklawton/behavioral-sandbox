...
//! `EBPF_KVM_ENGINE_VENDOR_DIR` to that dir takes every build path offline: [`fetch_one`](crate::artifacts)
...
/// **upstream** (it is what fills the mirror), regardless of any `EBPF_KVM_ENGINE_VENDOR_DIR` already set.
pub(crate) fn vendor(dir: Option<PathBuf>) -> Result<()> {
    ...
    println!(
        "\n✓ vendored {count} files + manifest in {}\n  build offline with: EBPF_KVM_ENGINE_VENDOR_DIR={} \
