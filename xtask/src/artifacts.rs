...
pub(crate) fn artifacts() -> Result<Vec<Artifact>> {
    ...
    match std::env::consts::ARCH {
        ...
        other => bail!(
            "no pinned artifacts for arch {other} yet (x86_64 only) — set EBPF_KVM_ENGINE_KERNEL/EBPF_KVM_ENGINE_ROOTFS \
...
/// Obtain one artifact into place. **Vendor-aware:** if `EBPF_KVM_ENGINE_VENDOR_DIR` is set, the artifact is
...
/// goes through here, so setting `EBPF_KVM_ENGINE_VENDOR_DIR` takes all of them offline at once.
    ...
    if !src.is_file() {
        bail!(
            "vendored input {name} not found in {} — run `cargo xtask vendor` to populate the \
             mirror (or unset EBPF_KVM_ENGINE_VENDOR_DIR to fetch from upstream)",
            vendor.display()
        );
    }
