...
use crate::{artifacts_dir, eke_rootfs_path, run_tool, run_tool_env, vendor_dir, workspace_root};
...
/// the contract vsock port (`eke_channel::EKE_VSOCK_PORT`, the same constant the host dials,
...
::sysinit:/sbin/net-up
ttyS0::respawn:/usr/local/bin/eke-guest vsock:{port}
::ctrlaltdel:/sbin/reboot
::shutdown:/bin/umount -a -r
",
        port = eke_channel::EKE_VSOCK_PORT
    )
}
...
/// Labels come from `eke-channel`, the one definition the driver (which stamps them) also uses.
...
",
        input = eke_channel::INPUT_LABEL,
        output = eke_channel::OUTPUT_LABEL,
    )
}
...
/// `eke_channel::GUEST_IP6_CMDLINE_KEY`, the one host↔guest definition the driver's writer and this
...
exit 0
",
        key = eke_channel::GUEST_IP6_CMDLINE_KEY,
    )
}
...
fn assemble_rootfs(out_image: &Path) -> Result<RootfsBuild> {
    ...
    // Bake the static agent in at /usr/local/bin/eke-guest.
    let bindir = staging.join("usr/local/bin");
    std::fs::create_dir_all(&bindir)?;
    let agent_dest = bindir.join("eke-guest");
...
pub(crate) fn build_rootfs(verify: bool, update_lock: bool) -> Result<()> {
    ...
    println!(
        "  exec inside a microVM with:\n  EKE_ROOTFS={} EKE_MARKER={} cargo run -p kee-cli -- run -- echo hi",
        out.display(),
        eke_channel::GUEST_READY_MARKER
    );
    Ok(())
}
...
/// root, no host `apk`. Vendor-aware: with `EKE_VENDOR_DIR` set it installs offline from the
...
mod tests {
    ...
    fn net_up_reads_the_shared_cmdline_key() {
        ...
        let key = eke_channel::GUEST_IP6_CMDLINE_KEY;
        // The guest parses the *same* token key the driver writes (single-sourced in `eke-channel`),
