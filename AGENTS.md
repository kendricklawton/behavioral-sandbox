# ebpf-kvm-engine: engineering disciplines
...
code can't see or subvert you. Every run yields a tamper-resistant, host-observed **audit log** of exactly what happened (host-signed for off-host detection, decision 034). This file is the operating manual, read it every session.
...
is always the *caller* driving the engine from outside, never an engine component (decision 031); the
...
crates/
  ...
  cli/           one entrypoint, the `ebpf-kvm-engine` CLI (also installed as `ebpf-kvm-engine`): (`run`,
                 `shell`, `doctor`; the audit record on
                 `--trace`/`--record`/`--record-summary`/`--watch`) plus `ebpf-kvm-engine serve`, the driver
...
- **Rust**, stable, one workspace. Linux-only (it needs KVM), `x86_64` only (decision 032 as
  ...
  **≥ 5.15** (a security-maintained LTS floor, untrusted code on an unpatched kernel is a
  threat-model hole; `ebpf-kvm-engine doctor` enforces it, decision 032). The eBPF programs build for their
...
- `tracing` logs to stderr; a run's structured result/audit-log to stdout, so
  `ebpf-kvm-engine run … 2>/dev/null` stays pipe-clean. Config is layered **flags > env (`EBPF_KVM_ENGINE_*`) >
  file (`.ebpf-kvm-engine.toml`, the nearest one walking up from the cwd) > defaults** (decision 027).
...
- **Git is human-driven.** The user makes every commit and push; the **coding agent** (Claude,
  Gemini, Codex, as opposed to `ebpf-kvm-engine`/ebpf-kvm-engine, this project) never runs `git commit` / `git push` (or
